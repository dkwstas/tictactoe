...
int add_to_section (section_t *section, char *text) {
	...
	temp_addr = (char **)realloc(section->text,
			(section->size + 1) * sizeof(char *));
	if (temp_addr != NULL) {
	...
	section->text[section->size] = 
		(char *)malloc((strlen(text) + 1) * sizeof(char));
...
int add_to_display (display_t *display, section_t *section) {
	...
	temp_addr = (section_t **)realloc(display->sections,
			(display->size + 1) * sizeof(section_t *));
	if (temp_addr != NULL) {
